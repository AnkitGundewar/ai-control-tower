...
class Supervisor:
    """
    Orchestrates the execution of all AI agents
    in the AI Control Tower workflow.
    """
    ...
    def execute(
        ...
        tracking_response = self.tracking_agent.run(
            request,
        )
        ...
        risk_request.payload["tracking"] = (
            tracking_response.payload
        )

        risk_response = self.risk_agent.run(
            risk_request,
        )
        ...
        root_cause_request = request.model_copy(
            deep=True,
        )

        root_cause_request.payload["tracking"] = (
            tracking_response.payload
        )
        ...
        recommendation_request = request.model_copy(
            deep=True,
        )

        recommendation_request.payload["tracking"] = (
            tracking_response.payload
        )

        recommendation_request.payload["riskAnalysis"] = (
            risk_response.payload
        )

        recommendation_request.payload["rootCauseAnalysis"] = (
            root_cause_response.payload
        )
        ...
        summary_request.payload["tracking"] = (
            tracking_response.payload
        )

        summary_request.payload["riskAnalysis"] = (
            risk_response.payload
        )

        summary_request.payload["rootCauseAnalysis"] = (
            root_cause_response.payload
        )

        summary_request.payload["recommendation"] = (
            recommendation_response.payload
        )

        executive_summary_response = (
            self.executive_summary_agent.run(
                summary_request,
            )
        )
